...
pub fn prove_ex<B: BackendForChannel<MC>, MC: MerkleChannel>(
    ...
    // Commit on the composition polynomial by recursively splitting its coeffs at the midpoint
    // `composition_log_split` times (left before right), and committing on each of the
    // `2^composition_log_split` parts separately. Each part has log degree equal to the maximal
    // trace log size.
    let composition_log_split = component_provers.components().composition_log_split();
    let mut composition_parts = vec![composition_poly];
    for _ in 0..composition_log_split {
        composition_parts = composition_parts
            .into_iter()
            .flat_map(|part| {
                let (left, right) = part.split_at_mid();
                [left, right]
            })
            .collect();
    }
    ...
    for part in composition_parts {
        tree_builder.extend_polys(part.into_coordinate_polys());
    }
