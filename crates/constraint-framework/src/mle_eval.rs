...
//!
//! A univariate IOP that proves an MLE evaluates to a claimed value at a point
//! (see <https://eprint.iacr.org/2023/1284.pdf>, Section 5.1). Promoted from the
//! Stwo `xor::gkr_lookups::mle_eval` example so downstream crates can commit a
//! GKR output claim (an MLE evaluation at the GKR-chosen OOD point) back to
//! trace columns. The `MleCoeffColumnOracle` abstraction lets the consumer
//! supply the evaluated MLE as a relation-combination of its committed columns.
//!
//! The prover component evaluates its quotient on a domain of log size
//! `log_size + composition_log_split` so its accumulated lift matches the
//! verifier's uniform lifting when mixed with higher-degree components (the
//! `8c998390` fix, retained here).
...
where
    F: Field,
    SecureField: stwo::core::fields::ExtensionOf<F>,
    B: stwo::prover::lookups::mle::MleOps<F>,
{
...
#[cfg(test)]
mod tests {
    const MIN_LOG_BLOWUP_FACTOR: u32 = 1;

    /// Test-only preprocessed step selector (was examples' `IsStepWithOffset`).
    #[derive(Debug)]
    struct IsStepWithOffset {
        log_size: u32,
        log_step: u32,
        offset: usize,
    }
    impl IsStepWithOffset {
        ...
        fn gen_column_simd(&self) -> CircleEvaluation<SimdBackend, BaseField, BitReversedOrder> {
            ...
            for i in (step_offset..size).step_by(step) {
                let cdi = coset_index_to_circle_domain_index(i, self.log_size);
                let cdi_bit_rev = bit_reverse_index(cdi, self.log_size);
                col.set(cdi_bit_rev, BaseField::one());
            }
            CircleEvaluation::new(CanonicCoset::new(self.log_size).circle_domain(), col)
        }
    }
